...
if st.session_state.get("process_query") and user_query.strip():
    ...
    with st.container():
        ...
        existing_cache_key = detected_company.lower().strip()
        if existing_cache_key in st.session_state.company_stores:
            document, store = st.session_state.company_stores[existing_cache_key]
            ...
            if not pdf_url or not filepath:
                st.error(
                    ...
                    "Try the official company name or a well-known ticker like TCS."
                )
                st.stop()

            canonical_company = (company_name or detected_company).strip()
            company_key = canonical_company.lower()
            st.write(f"Source URL: {pdf_url}")

            metadata = DocumentMetadata(
                company_name=canonical_company.title(),
                document_type="Integrated Annual Report",
                financial_year="2024-25",
                source_url=pdf_url,
                source_domain=urlparse(pdf_url).netloc,
                search_query=f"{canonical_company} Integrated Annual Report 2024-25 filetype:pdf",
            )
        ...
        company_key = document.metadata.company_name.lower().strip()
